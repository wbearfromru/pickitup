...
var openFB = (function () {
    ...
    // Indicates that app is running with Intel XDK
        runningInIntel,
...
//    }, false);
    ...
    function init(appId, redirectURL, store) {
        ...
        sessionStorage.removeItem("myOrigin");
    }
    ...
    function login(scope, success, error) {
        ...
        function loginWindowLoadStart(event) {
            ...
            if (url.indexOf("access_token=") > 0 || url.indexOf("error=") > 0) {
                ...
                oauthCallback(url, success, error);
            }
        }
        ...
        if (!oauthRedirectURL) {
            if (runningInCordova || runningInIntel) {
        ...
        var urlToOpen = FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + encodeURIComponent(oauthRedirectURL) + '&response_type=token&display=popup&scope=' + scope;
//        if (runningInIntel){
//        	intel.xdk.device.showRemoteSite(urlToOpen,0,0,50,50);
//        } else {
        	location.href = 'http://pickitupbasketball.co/facebooklogin.html?origin=' + encodeURIComponent(typeof intel != 'undefined' && intel.xdk.webRoot && intel.xdk.webRoot.length > 0 ? intel.xdk.webRoot : window.location.origin) + '&call=' + encodeURIComponent(urlToOpen);
        	//loginWindow = window.open(urlToOpen, '_blank', 'location=no');
//        }
        
		if (runningInIntel) {
        	var previousUrl = urlToOpen;
	        var checkInterval = setInterval(
	        	function(){
	        		var currentUrl = loginWindow.location.href;
	        		if (currentUrl != previousUrl){
	        			console.log('Url changed: ' + currentUrl);
	        			previousUrl = currentUrl;
	        			loginWindowLoadStart({url: currentUrl});
	        		}
	        	}, 1);
        }
     ...
     * OAuth workflow.
     */
    function oauthCallback(url, loginSuccessHandler, loginErrorHandler ) {
